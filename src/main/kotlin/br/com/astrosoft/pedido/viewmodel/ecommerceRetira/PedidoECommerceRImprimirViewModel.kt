...
class PedidoECommerceRImprimirViewModel(val viewModel: PedidoECommerceRViewModel) {
  ...
  private fun listPedidosECommerceImprimir(): List<Pedido> {
    ...
    return Pedido
      .listaPedidoImprimir(FiltroPedido(tipo = RETIRA, ecommerce = true, dataInicial = null, dataFinal = null))
      .filter { pedido ->
  ...
  fun confirmaPrint() {
    val pedidos = viewModel.view.tabECommerceRImprimir.itensSelecionado().ifEmpty { fail("Não há pedido selecionado") }
