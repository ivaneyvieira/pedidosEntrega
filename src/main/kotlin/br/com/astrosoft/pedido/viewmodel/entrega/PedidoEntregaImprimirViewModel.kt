...
class PedidoEntregaImprimirViewModel(val viewModel: PedidoEntregaViewModel) {
  ...
  private fun listPedidosEntregaImprimir(): List<Pedido> {
    ...
    return Pedido
      .listaPedidoImprimir(FiltroPedido(tipo = ENTREGA, ecommerce = false, dataInicial = null, dataFinal = null))
      .filter { pedido ->
  ...
  fun confirmaPrint() {
    val pedidos = viewModel.view.tabEntregaImprimir.itensSelecionado().ifEmpty { fail("Não há pedido selecionado") }
