...
class PedidoECommerceEImprimirViewModel(val viewModel: PedidoECommerceEViewModel) {
  private val subView
    get() = viewModel.view.tabECommerceEImprimir
  ...
  private fun listPedidosECommerceImprimir(): List<Pedido> {
    ...
    return Pedido
      .listaPedidoImprimir(FiltroPedido(tipo = ENTREGA, ecommerce = true, dataInicial = null, dataFinal = null))
      .filter { pedido ->
  ...
  fun confirmaPrint() {
    val pedidos = viewModel.view.tabECommerceEImprimir.itensSelecionado().ifEmpty { fail("Não há pedido selecionado") }
...
interface IPedidoECommerceEImprimir {
